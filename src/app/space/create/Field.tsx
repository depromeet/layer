...
import { CategoryButton } from "@/component/space/CategoryButton";
import { spaceState } from "@/store/space/spaceAtom";
import { fieldArr, fieldMap, FieldType, SpaceValue } from "@/types/space";
...
export function Field({ onNext }: { onNext: (categoryValues: Pick<SpaceValue, "field">) => void }) {
  ...
  return (
    <Fragment>
      ...
      >
        {fieldArr.map((item) => (
          <CategoryButton key={item} label={fieldMap[item]} onClick={() => handleButtonClick(item)} isChecked={checkedField.includes(item)} />
        ))}
      </div>
      <ButtonProvider>
