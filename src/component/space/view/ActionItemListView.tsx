...
type TeamGoalViewPros = {
  teamActionList: ActionItemType[] | undefined;
};
...
export function ActionItemListView({ teamActionList }: TeamGoalViewPros) {
  ...
  const handleAddActionItem = ({ retrospectId, content }: PostActionItemProps) => {
    if (textValue.trim() === "") {
      return;
    }
    postActionItem({ retrospectId: retrospectId, content: content });
...
"
      ...
      <div
        ...
        `}
      >
        {teamActionList?.map((teamActionItem) => <ActionItem key={teamActionItem.actionItemId} content={teamActionItem.actionItemContent} />)}

        {Array.from({ length: 3 - (teamActionList?.length || 0) }).map((_, index) => (
          ...
          <Fragment>
            ...
            >
              ...
              <Button
                colorSchema="black"
                onClick={() => {
                  handleAddActionItem({ retrospectId: "100", content: textValue });
                }}
              >
                추가하기
              </Button>
...
function ActionItem({ content }: ActionItemProps) {
  return (
    ...
    >
      ...
      <Typography variant="B2_MEDIUM">{content}</Typography>
    </div>
  );
}
